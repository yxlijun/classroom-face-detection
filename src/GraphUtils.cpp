...
IplImage* drawIntGraph(const int *arraySrc, int nArrayLength, CvScalar color, IplImage *imageDst)
{
	int b = 80;
	int minV = 0;
	int maxV = 100;
	int w = imageDst->width;
	int h = 220;
	int s = 220;
	float xscale = 1800 / 610;
	...
	int min = 220;
	//cvLine(imageGraph, cvPoint(b, min-(b)), cvPoint(w - b, min-b), BLACK);
	//cvLine(imageGraph, cvPoint(b, min - (b)), cvPoint(b, min - (b + s)), BLACK);
	...
	// Draw the values
	CvPoint ptPrev = cvPoint(b, min - (arraySrc[0] * fscale));	// Start the lines at the 1st point.
	for (int i = 1; i<nArrayLength; i++) {
		int y = cvRound((arraySrc[i] - minV) * fscale);	// Get the values at a bigger scale
		int x = cvRound(i * xscale);
		CvPoint ptNew = cvPoint(b + x, min - y);
		cvLine(imageGraph, ptPrev, ptNew, color, 2, CV_AA);	// Draw a line from the previous point to the new point
		ptPrev = ptNew;
	}
